...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.Picture.Intercept(interceptors...)
}

// Mutate implements the ent.Mutator interface.
func (c *Client) Mutate(ctx context.Context, m Mutation) (Value, error) {
	switch m := m.(type) {
	case *PictureMutation:
		return c.Picture.mutate(ctx, m)
	default:
...
// A call to `Intercept(f, g, h)` equals to `picture.Intercept(f(g(h())))`.
func (c *PictureClient) Intercept(interceptors ...Interceptor) {
	c.inters.Picture = append(c.inters.Picture, interceptors...)
}
...
func (c *PictureClient) MapCreateBulk(slice any, setFunc func(*PictureCreate, int)) *PictureCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &PictureCreateBulk{err: fmt.Errorf("calling to PictureClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*PictureCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *PictureClient) Query() *PictureQuery {
	return &PictureQuery{
		config: c.config,
		ctx:    &QueryContext{Type: TypePicture},
...
func (c *PictureClient) Interceptors() []Interceptor {
	return c.inters.Picture
}

func (c *PictureClient) mutate(ctx context.Context, m *PictureMutation) (Value, error) {
	switch m.Op() {
	case OpCreate:
		return (&PictureCreate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdate:
		return (&PictureUpdate{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpUpdateOne:
		return (&PictureUpdateOne{config: c.config, hooks: c.Hooks(), mutation: m}).Save(ctx)
	case OpDelete, OpDeleteOne:
		return (&PictureDelete{config: c.config, hooks: c.Hooks(), mutation: m}).Exec(ctx)
	default:
		return nil, fmt.Errorf("ent: unknown Picture mutation op: %q", m.Op())
	}
}
